<?php

namespace App\Http\Controllers\v1;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Log;
use App\Models\v1\Stock;
use App\Models\v1\StockRecommendationTrends;

class StockRecommendationController extends Controller
{
    /**
     * The base URL of the Finnhub API.
     *
     * @var string
     */
    private $apiBaseUrl;

    /**
     * Your Finnhub API key.
     *
     * @var string
     */
    private $apiKey;

    /**
     * Controller constructor.
     */
    public function __construct()
    {
        $this->apiBaseUrl = config('services.finnhub.base_url');
        $this->apiKey = config('services.finnhub.key');
    }

    /**
     * Get stock recommendation trends for a stock symbol.
     *
     * @param string $symbol
     * @return bool|null
     */
    public function getStockRecommendation(string $symbol)
    {
        // Convert the symbol to uppercase
        $symbol = strtoupper($symbol);

        $response = $this->fetchStockRecommendationFromAPI($symbol);

        if ($response) {
            $this->processAndSaveStockRecommendation($response, $symbol);
            return $response;
        } else {
            //Log::warning("Failed to fetch stock recommendation trends for symbol: $symbol");
            return null;
        }
    }

    /**
     * Get stock recommendation trends for all stock symbols.
     *
     * @param Request $request
     * @return void
     */
    public function getStockRecommendationBatch(Request $request)
    {
        $symbol = $request->input('symbol');
        $this->getStockRecommendation($symbol);
    }

    /**
     * Make an API request to the Finnhub service.
     * Fetch stock recommendation trends for a stock symbol.
     *
     * @param string $symbol The stock symbol.
     * @return array|null The API response data or null if an error occurred.
     */
    private function fetchStockRecommendationFromAPI(string $symbol): ?array
    {
        $endpoint = '/stock/recommendation';
        $params = [
            'symbol' => $symbol,
            'token' => $this->apiKey,
        ];

        try {
            $response = Http::timeout(15)->get($this->apiBaseUrl . $endpoint, $params);

            if ($response->failed()) {
               // Log::error('API request failed.', [
                //    'status' => $response->status(),
               //     'error' => $response->json(),
               // ]);
                return null;
            }

            return $response->json();
        } catch (\Exception $e) {
           // Log::error('An exception occurred during API request.', [
           //     'endpoint' => $endpoint,
           //     'message' => $e->getMessage(),
           //     'trace' => $e->getTraceAsString(),
           // ]);
            return null;
        }
    }

    /**
     * Process and save the stock recommendation trends data.
     *
     * @param array $response
     * @param string $symbol
     */
    public function processAndSaveStockRecommendation($response, $symbol)
    {
        $stockId = Stock::where('symbol', $symbol)->value('id');
        //$recommendations = $response['data'];

        if ($stockId) {
            foreach ($response as $recommendation) {
                StockRecommendationTrends::updateRecommendationTrends(
                    $stockId,
                    $recommendation
                );
            }
        } else {
            //Log::warning("Stock not found for symbol: " . $symbol);
            return null;
        }
    }
}