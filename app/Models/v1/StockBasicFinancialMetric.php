...
class StockBasicFinancialMetric extends Model
{
    ...
    {       
        return (bool) self::updateOrCreate(
            ...
            [
                ...
                'netmargin' => $metric['metric']['netmargin'][0]['v'] ?? 0,
                ...
                'revenueGrowthTTMYoy'=>$metric['metric']['revenueGrowthTTMYoy'] ?? 0,
                'ebitdPerShareTTM'=> $metric['metric']['ebitdPerShareTTM'] ?? 0
            ]
        );
    }
    ...
    /**
     * Get basic financial metrics data for a specific stock ID.
     *
     * @param int $stockId
     * @return array
     */
    public static function getBasicFinancialMetricsByStockId(int $stockId): array
    {
        return self::where('stock_id', $stockId)
            ->join('stock_symbols', 'stock_basic_financials_metric.stock_id', '=', 'stock_symbols.id')
            ->get(['stock_basic_financials_metric.*', 'stock_symbols.symbol'])
            ->toArray();
    }
}