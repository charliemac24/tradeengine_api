...
class ProcessStocksTradingScore extends Command
{
     ...
     * @var string
     */
    protected $signature = 'stocks:scoring';
     ...
     * @var string
     */
    protected $description = 'Process stock score (stocks_commands) only where score=0, cycling flags each run.';
     ...
     * Regardless of API success/failure, processed_score is set to 1 for the symbols
    ...
    {
        // Start of cycle: if no pending (score=0), reset all to 0 so a new full pass begins.
        if (!DB::table($this->table)->where('score', 0)->exists()) {
            DB::table($this->table)->update(['score' => 0]);
            $this->info('No pending symbols. Reset all score to 0 (new cycle started).');
        }
        ...
        DB::table($this->table)->update(['score' => 1]);

        $this->info(sprintf(
            'Processed %d symbols (successfully flagged this run: %d, max per run: %d). All score now set to 1.',
            count($symbols),
            $totalSuccess,
            $this->maxPerRun
        ));
