...
class ProcessStockInsiderSentiment extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'stocks:insider_sentiment';
     ...
     * @var string
     */
    protected $description = 'Process stock insider_sentiment (stocks_commands) only where insider_sentiment=0, cycling flags each run.';
     ...
     * Regardless of API success/failure, insider_sentiment is set to 1 for the symbols
    ...
    {
        // Start of cycle: if no pending (insider_sentiment=0), reset all to 0 so a new full pass begins.
        if (!DB::table($this->table)->where('insider_sentiment', 0)->exists()) {
            DB::table($this->table)->update(['insider_sentiment' => 0]);
            $this->info('No pending symbols. Reset all insider_sentiment to 0 (new cycle started).');
        }
        ...
        DB::table($this->table)->update(['insider_sentiment' => 1]);

        $this->info(sprintf(
            'Processed %d symbols (successfully flagged this run: %d, max per run: %d). All insider_sentiment now set to 1.',
            count($symbols),
            $totalSuccess,
            $this->maxPerRun
        ));
