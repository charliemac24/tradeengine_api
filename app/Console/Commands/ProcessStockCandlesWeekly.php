...
class ProcessStockCandlesWeekly extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'stocks:candles_weekly';
     ...
     * @var string
     */
    protected $description = 'Process stock weekly candles in batches using concurrent API calls.';
    ...
    public function handle(): int
    {
        ...
        if (!empty($symbols)) {
            ...
            // If no symbols left, reset all processed_candles_weekly to 0
            DB::table('stocks_by_market_cap')->update(['processed_candles_weekly' => 0]);
        }
    ...
    {
        // Directly fetch symbols from stocks_by_market_cap where processed_candles_weekly = 0 AND notpriority = 0
        return DB::table('stocks_by_market_cap')
            ->where('processed_candles_weekly', 0)
            ->where('notpriority', 0)
            ->pluck('symbol')
            ->toArray();
    }
    ...
    protected function processSymbol(string $symbol): void
    {
        ...
        // After processing, set processed_candles_weekly = 1 for this symbol
        DB::table('stocks_by_market_cap')
            ->where('symbol', $symbol)
            ->update(['processed_candles_weekly' => 1]);
    }
