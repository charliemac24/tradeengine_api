...
class ProcessStocksIndicatorsBatch1 extends Command
{
     ...
     * @var string
     */
    protected $signature = 'stocks:indicator_non_historical_p1';
     ...
     * @var string
     */
    protected $description = 'Process stock indicator_non_historical_p1 (stocks_commands) only where indicator_non_historical_p1=0, cycling flags each run.';
     ...
     * Regardless of API success/failure, processed_indicator_non_historical_p1 is set to 1 for the symbols
    ...
    {
        // Start of cycle: if no pending (indicator_non_historical_p1=0), reset all to 0 so a new full pass begins.
        if (!DB::table($this->table)->where('indicator_non_historical_p1', 0)->exists()) {
            DB::table($this->table)->update(['indicator_non_historical_p1' => 0]);
            $this->info('No pending symbols. Reset all indicator_non_historical_p1 to 0 (new cycle started).');
        }
        ...
        foreach ($chunks as $chunk) {
            ...
            if ($successfulSymbols) {
                DB::table($this->table)
                    ...
                    ->update(['indicator_non_historical_p1' => 1]);
        ...
        DB::table($this->table)->update(['indicator_non_historical_p1' => 1]);

        $this->info(sprintf(
            'Processed %d symbols (successfully flagged this run: %d, max per run: %d). All indicator_non_historical_p1 now set to 1.',
            count($symbols),
            $totalSuccess,
            $this->maxPerRun
        ));
    ...
    protected function getStockSymbols(int $limit = 2000): array
    {
        return DB::table($this->table)
            ->where('indicator_non_historical_p1', 0)
            ->orderBy('id', 'asc')
            ->limit($limit)
            ->pluck('symbol')
            ->toArray();
