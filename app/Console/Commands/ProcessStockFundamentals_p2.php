...
class ProcessStockfundamentals_p2 extends Command
{
     ...
     * @var string
     */
    protected $description = 'Process stock fundamentals_p2 (stocks_commands) only where fundamentals_p2=0, cycling flags each run.';
     ...
     * Regardless of API success/failure, processed_fundamentals_p2 is set to 1 for the symbols
    ...
    {
        // Start of cycle: if no pending (fundamentals_p2=0), reset all to 0 so a new full pass begins.
        if (!DB::table($this->table)->where('fundamentals_p2', 0)->exists()) {
            DB::table($this->table)->update(['fundamentals_p2' => 0]);
            $this->info('No pending symbols. Reset all fundamentals_p2 to 0 (new cycle started).');
        }
        ...
        DB::table($this->table)->update(['fundamentals_p2' => 1]);

        $this->info(sprintf(
            'Processed %d symbols (successfully flagged this run: %d, max per run: %d). All fundamentals_p2 now set to 1.',
            count($symbols),
            $totalSuccess,
            $this->maxPerRun
        ));
